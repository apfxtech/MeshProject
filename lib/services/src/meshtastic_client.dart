...
class MeshtasticClient {
  ...
  final StreamController<Channel> _channelController = 
    StreamController<Channel>.broadcast();
  final StreamController<Config> _configPartController = 
      StreamController<Config>.broadcast();
  final StreamController<ModuleConfig> _moduleConfigPartController = 
      StreamController<ModuleConfig>.broadcast();
  ...
  Stream<Config> get configPartStream => _configPartController.stream;
  Stream<ModuleConfig> get moduleConfigPartStream => _moduleConfigPartController.stream;
  ...
  Future<void> connectToDevice(BluetoothDevice device) async {
    ...
    try {
      ...
      // Connect to device
      await device.connect(timeout: const Duration(seconds: 30), license: License.free);
  ...
  void dispose() {
    ...
    _configPartController.close();
    _moduleConfigPartController.close();
  }
}
