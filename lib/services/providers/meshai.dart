...
class MeshtasticProvider extends LlmProvider with ChangeNotifier {
  ...
  Stream<String> generateStream(String prompt, {Iterable<Attachment> attachments = const []}) {
    return Stream.value('Dummy response to: $prompt');
  }
  ...
  Stream<String> sendMessageStream(String prompt, {Iterable<Attachment> attachments = const []}) async* {
    ...
    if (isMesh) {
      ...
      final response = 'Dummy reply to: $prompt';
      final llmMessage = ChatMessage(origin: MessageOrigin.llm, text: '', attachments: const []);
      _history.add(llmMessage);
      notifyListeners();
      final dbLlmMsg = _chatMessageToMessage(llmMessage, role: 'assistant');
      final msgId = await ChatsRepository.messages(chatId!).add(dbLlmMsg);

      final words = response.split(' ');
      for (final word in words) {
        await Future.delayed(const Duration(milliseconds: 100));
        llmMessage.append('$word ');
        notifyListeners();
        dbLlmMsg.content += '$word ';
        await ChatsRepository.messages(chatId!).edit(msgId, dbLlmMsg.content);
        yield '$word ';
      }
    }
